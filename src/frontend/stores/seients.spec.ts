...
function makeSocketMock() {
  ...
  return {
    ...
    off: vi.fn(),
    triggerEvent: (event: string, payload: unknown) => handlers[event]?.(payload),
  };
}
...
describe('useSeientStore', () => {
  ...
  describe('actualitzarEstat', () => {
    ...
    it('no fa res si el seatId no existeix al Map', async () => {
      ...
      expect(() => store.actualitzarEstat('seat-inexistent', EstatSeient.VENUT)).not.toThrow();
      expect(store.llistat.size).toBe(3);
    });
  ...
  describe('connectar', () => {
    ...
    it('subscriu el listener seient:canvi-estat', async () => {
      ...
      expect(socketMock.on).toHaveBeenCalledWith('seient:canvi-estat', expect.any(Function));
    });
