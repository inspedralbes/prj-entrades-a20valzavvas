...
export class ReservationsScheduler {
  ...
  async releaseExpired(): Promise<void> {
    try {
      const { released } = await this.laravelClient.releaseExpiredReservations();

      const affectedEventIds = new Set<string>();

      for (const { seatId, eventId } of released) {
        this.seatsGateway.server.to(`event:${eventId}`).emit('seient:canvi-estat', {
          seatId,
      ...
      for (const eventId of affectedEventIds) {
        try {
          ...
          const sockets = await this.seatsGateway.server.in(`event:${eventId}`).fetchSockets();
