...
describe('JwtWsGuard', () => {
  ...
  beforeEach(() => {
    ...
    guard = new JwtWsGuard(mockJwtService, mockConfigService, mockLaravelClient);
  });
  ...
  it('should return true and set socket.data when token is valid', async () => {
    ...
    const client = ctx.switchToWs().getClient<{ data: Record<string, string> }>();
  ...
  it('should throw WsException when no token is provided', async () => {
    ...
    await expect(guard.canActivate(ctx)).rejects.toThrow(new WsException('Unauthorized'));
  });
  ...
  describe('Sanctum opaque token fallback', () => {
    it('should authenticate with a valid Sanctum opaque token via Laravel', async () => {
      ...
      expect(result).toBe(true);
      expect(mockGetUserByToken).toHaveBeenCalledWith('11|sanctumOpaqueTokenAbc123');
