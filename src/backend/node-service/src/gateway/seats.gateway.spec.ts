...
import type { SeientCanviEstatPayload, StatsActualitzacioPayload } from 'shared/types/socket.types';
...
describe('SeatsGateway', () => {
  ...
  describe('emitStatsActualitzacio', () => {
    ...
    it('el payload conté tots els camps obligatoris', () => {
      const { server, to: _to, emit } = makeServerMock();
      (gateway as unknown as { server: typeof server }).server = server;
  ...
  describe('handleSeientAlliberar', () => {
    it("emet seient:canvi-estat DISPONIBLE a la room quan l'alliberament és correcte", async () => {
      ...
      expect(laravelClient.releaseSeat).toHaveBeenCalledWith('seat-C3', 'user-1', 'tok-abc');
      expect(to).toHaveBeenCalledWith('event:evt-1');
      expect(roomEmit).toHaveBeenCalledWith(
        'seient:canvi-estat',
        expect.objectContaining({
      ...
      expect(socket.emit).not.toHaveBeenCalledWith('error:general', expect.any(Object));
    });
