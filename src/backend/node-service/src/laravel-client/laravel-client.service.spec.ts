...
function createMockHttpService() {
  ...
  const httpService = {
    ...
    axiosRef: {
      interceptors: {
        response: {
          use: vi.fn((fulfilled: (v: any) => any, rejected: (e: any) => any) => {
...
describe('LaravelClientService', () => {
  ...
  describe('error mapping', () => {
    ...
    it('should map 422 to UnprocessableEntityException', () => {
      ...
      expect(() => errorInterceptor(error)).toThrow(UnprocessableEntityException);
    });
    ...
    it('should map 500 to InternalServerErrorException', () => {
      ...
      expect(() => errorInterceptor(error)).toThrow(InternalServerErrorException);
    });
  ...
  describe('releaseSeat', () => {
    it('retorna ok:true quan Laravel respon 204', async () => {
      ...
      expect(result.ok).toBe(true);
      expect(httpService.delete).toHaveBeenCalledWith('/api/seats/seat-C3/reserve', {
        headers: { Authorization: 'Bearer tok-abc' },
      });
    });
  ...
  describe('releaseExpiredReservations', () => {
    it('retorna la llista de seients alliberats quan Laravel respon 200', async () => {
      ...
      expect(httpService.delete).toHaveBeenCalledWith('/internal/reservations/expired', {
