...
const stubWsGuard: CanActivate = {
  canActivate(ctx: ExecutionContext): boolean {
    ...
    const token = (client.handshake.auth as Record<string, string>)?.token ?? 'test-token';
...
describe('Concurrència: dos clients Socket.IO no poden reservar el mateix seient', () => {
  ...
  beforeAll(async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      client1.on('connect', () => {
        console.log('[TEST] client1 connected');
        onConnect();
      });
      client1.on('connect_error', (e) => {
        console.error('[TEST] client1 connect_error:', e.message);
        onError(e);
      });
      client2.on('connect', () => {
        console.log('[TEST] client2 connected');
        onConnect();
      });
      client2.on('connect_error', (e) => {
        console.error('[TEST] client2 connect_error:', e.message);
        onError(e);
      });
    });
  ...
  it("exactament un client rep reserva:confirmada i l'altre reserva:rebutjada", async () => {
    ...
    const [r1Promise, r2Promise] = [makeListener(client1), makeListener(client2)];
