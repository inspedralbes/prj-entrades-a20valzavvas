...
Aquest spec defineix els requisits del test d'integració automatitzat que verifica de manera determinista que el sistema de reserves de seients en temps real prevé correctament condicions de cursa (race conditions). L'objectiu és garantir que dos clients simultanis no puguin reservar el mateix seient, validant la lògica de `SELECT FOR UPDATE` de `SeatsService` contra una base de dades PostgreSQL real.
