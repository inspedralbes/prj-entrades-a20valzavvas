...
This capability handles real-time notification of concurrent seat reservation conflicts. When a buyer's seat reservation is rejected because another buyer reserved it first, the system detects the conflict event, updates the seat map optimistically, and renders a non-blocking toast notification to inform the buyer and prompt them to choose a different seat.
