...
El sistema SHALL exposar l'endpoint `POST /api/orders` (autenticat via Sanctum) que crea una ordre amb tots els seients reservats de l'usuari, els marca com a `VENUT` i elimina les reserves, tot dins d'una transacció de base de dades.

#### Scenario: Compra completada amb èxit
- **GIVEN** que l'usuari autenticat té N reserves actives no expirades
- **WHEN** envia `POST /api/orders` amb `{nom: "Joan", email: "joan@ex.com"}`
- **THEN** el sistema retorna HTTP 201 amb `{id, total_amount, items}`, crea un `Order` + N `OrderItem`s, actualitza cada `Seat.status` a `VENUT`, i elimina les N `Reservation`s
